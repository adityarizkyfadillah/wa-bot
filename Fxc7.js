...
const {
   ...
   mentionedJid,
   processTime,
} = require('@adityaganteng/PlanetPluto')
...
// json read file
const { BarBarApi, ZeksApi, TobzApi, VthearApi, LolApi, adityaApi } = JSON.parse(fs.readFileSync('./database/json/apikey.json'))
...
//function
const zalgo = require('./aditya/zalgo')
const { fetchaditya, fetchText } = require('./lib/fetcher')
...
function addMetadata(packname, author) {
				if (!packname) packname = 'termux-bot-wa'; if (!author) author = ' aditya';
...
async function starts() {
	...
	fs.existsSync('./aditya.json') && frhan.loadAuthInfo('./aditya.json')
	frhan.on('connecting', () => {
		start('2', 'Connecting...')
	})
	frhan.on('open', () => {
	...
	await frhan.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./aditya.json', JSON.stringify(frhan.base64EncodedAuthInfo(), null, '\t'))
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const aditya = Link.slice(0).trim().split(/ +/).shift().toLowerCase()
			...
			const adityaGans = ["0@s.whatsapp.net"]
			const adityaGans2 = " ~ 𝐂𝐫𝐞𝐚𝐭𝐞𝐝 𝐁𝐲 𝐅𝐚𝐫𝐡𝐚𝐧𝐗𝐂𝐨𝐝𝐞𝟳"
			const isUrl = (url) => {
...
if (isGroup) {
		...
		if (aditya.includes("://chat.whatsapp.com/")){
		if (!isGroup) return
...
return new Promise(resolve => setTimeout(resolve, ms));
		...
		break
			...
			case 'brainly':
					...
					costum(teks, text, adityaGans, `Ciee Cari Jawaban Yaa😂\nFollow IG: @_farhan_xcode7`)
					console.log(res)
					})
					await limitAdd(sender)
				break 
		...
		case 'verify':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				user.push(sender)
				costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal(), groupName, premi, Simihh, Welcomee, ModeAnime, Nsfww, BadWordd, AntiLinkk, AntiVirtexx), text, adityaGans, adityaGans2)
			break 

// fitur simple
		case 'listsurah':
				await costum(listsurah(), text, adityaGans, adityaGans2)
			break 
			case 'menumedia':
				user.push(sender)
				await costum(mediaa(prefix, pushname2, groupName, name), text, adityaGans, adityaGans2)
			break 
			case 'menucreator':
				user.push(sender)
				await costum(creator(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menufun':
				user.push(sender)
				await costum(fun(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menuspam':
				user.push(sender)
				await costum(spam(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menuinformasi':
				user.push(sender)
				await costum(informasi(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menuprimbon':
				user.push(sender)
				await costum(primbon(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menuencrypt':
				user.push(sender)
				await costum(encrypt(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menugroup':
				user.push(sender)
				uptime = process.uptime()
				await costum(groupp(prefix, pushname2, groupName, user, name, uptime, jam, tanggal(), premi, Simihh, Welcomee, ModeAnime, Nsfww, BadWordd, AntiLinkk, AntiVirtexx), text, adityaGans, adityaGans2)
			break 
			case 'menupremium':
				user.push(sender)
				premium.push(sender)
				await costum(menupremium(prefix, pushname2, groupName, user, name, premi), text, adityaGans, adityaGans2)
			break 
			case 'menuothers':
				user.push(sender)
				await costum(others(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menuowner':
				user.push(sender)
				await costum(ownerrr(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
			case 'menuscrapper':
				user.push(sender)
				await costum(scrapper(prefix, pushname2, groupName, user, name), text, adityaGans, adityaGans2)
			break 
// end fitur simple

			case 'infobot':
				await costum(bottt(prefix), text, adityaGans, botinfo)
					break
				...
				case 'sticker':
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('adityaXCode7', 'Jangan Lupa Donasi')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									await costum(fs.readFileSync(ran), sticker, adityaGans, ` ~ Nihh Udah Jadi Stikernya`)
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('adityaXCode7', 'Jangan Lupa Donasi')} ${ran} -o ${ran}`, async (error) => {
									if (error) return reply(mess.error.stick)
									await costum(fs.readFileSync(ran), sticker, adityaGans, `~ Nih Dah Jadi Gif Stikernya`)
									fs.unlinkSync(media)
									fs.unlinkSync(ran)
								})
							})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('adityaXCode7', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
					...
					fs.unlinkSync(ranp)
					if (err) return reply(mess.error.stick)
					exec(`webpmux -set exif ${addMetadata('adityaXCode7', 'Jangan Lupa Donasi')} ${rano} -o ${rano}`, async (error) => {
					if (error) return reply(mess.error.stick)
			...
			case 'kalkulator':
				...
				mtk = `${body.slice(12)}`
				try {
				anu = await fetchaditya(`https://api.vhtear.com/calculator?value=${mtk}&apikey=${VthearApi}`, {method: 'get'})
			...
			break
				...
				case 'animecry':
					cry = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/cry?apikey=${TobzApi}`, {method: 'get'})
					if (!isUser) return reply(mess.only.userB)
				...
				case 'animehug':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/hug?apikey=${TobzApi}`, {method: 'get'})
					if (!isUser) return reply(mess.only.userB)
			...
			break 
				...
				kode = ts.split("/")[0]
				teks = ts.split("/")[1]
				anu = await fetchaditya(`https://api.arugaz.my.id/api/edu/translate?lang=${kode}&text=${teks}`)
				reply(mess.wait)
			...
			case 'truth':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				anu = await fetchaditya(`https://xptnbotapinew.herokuapp.com/?truth&apikey=xptn`, {method: 'get'})
				ttrth = `${anu.Dare}`
			...
			break 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				anu = await fetchaditya(`https://xptnbotapinew.herokuapp.com/?dare&apikey=xptn`, {method: 'get'})
				der = `${anu.Dare}`
			...
			case 'tagme':
				...
				if (!isUser) return reply(mess.only.userB)
				await costum(`@${sender.split('@')[0]}`, text, adityaGans, `@${sender.split('@')[0]}`)
				break
			case 'lirik':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				teks = body.slice(7)
				try {
				anu = await fetchaditya(`http://scrap.terhambar.com/lirik?word=${teks}`, {method: 'get'})
				await costum(monospace('Lirik dari lagu '+teks+' adalah :\n\n'+anu.result.lirik), text, adityaGans, adityaGans2)
				} catch { 
				reply(mess.error.bug)
				}
				await limitAdd(sender) 
			...
			case 'memeindo':
				...
				reply(mess.wait)
				try {
				memein = await fetchaditya(`https://api.zeks.xyz/api/memeindo?apikey=${ZeksApi}`, {method: 'get'})
			...
			case 'meme':
				...
				reply(mess.wait)
				try {
				meme = await fetchaditya(`https://some-random-api.ml/meme`)
			...
			case 'nsfwloli':
				...
				res = await fetchaditya(`https://api.vhtear.com/randomloli&apikey=${VthearApi}`, {method: 'get'})
			...
			case 'nsfwsolo':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
				if (!isNsfw) return reply(' *FALSE* ')
				res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/solo?apikey=${LolApi}`, {method: 'get'})
			...
			case 'nsfweron':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
				if (!isNsfw) return reply(' *FALSE* ')
				res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/eron?apikey=${LolApi}`, {method: 'get'})
				buffer = await getBuffer(res.result)
				frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
				} catch (e) {
				console.log(`Error :`, color(e,'red'))
			...
			break 
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
				if (!isNsfw) return reply(' *FALSE* ')
				res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/lewd?apikey=${LolApi}`, {method: 'get'})
				buffer = await getBuffer(res.result)
				frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
				} catch (e) {
				console.log(`Error :`, color(e,'red'))
			...
			case 'nsfwyuri':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
				if (!isNsfw) return reply(' *FALSE* ')
				res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/yuri?apikey=${LolApi}`, {method: 'get'})
				buffer = await getBuffer(res.result)
				frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
				} catch (e) {
				console.log(`Error :`, color(e,'red'))
			...
			break 
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
				if (!isNsfw) return reply(' *FALSE* ')
				res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random/nsfw/ecchi?apikey=${LolApi}`, {method: 'get'})
				buffer = await getBuffer(res.result)
				frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
				} catch (e) {
				console.log(`Error :`, color(e,'red'))
			...
			break 
				case 'nsfwparadise':
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random/nsfw/hentaiparadise?apikey=${LolApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'nsfwbigtt':
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random/nsfw/biganimetiddies?apikey=${LolApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'nsfwclasic':
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/classic?apikey=${LolApi}`, {method: 'get'})
						buffer = await getBuffer(res.result.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'nsfwcum':
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/cum_jpg?apikey=${LolApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'nsfwpussy':
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`http://lolhuman.herokuapp.com/api/random2/pussy?apikey=${LolApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
			    ...
			    case 'nsfwblowjob':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`https://tobz-api.herokuapp.com/api/nsfwblowjob?apikey=${TobzApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
			    ...
			    case 'nsfwneko':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`https://tobz-api.herokuapp.com/api/nsfwneko?apikey=${TobzApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
				...
				case 'nsfwtrap':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`https://tobz-api.herokuapp.com/api/nsfwtrap?apikey=${TobzApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'hentai':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE* ')
						res = await fetchaditya(`https://tobz-api.herokuapp.com/api/hentai?apikey=${TobzApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				try {
					...
					tels = body.slice(7)
					anu = await fetchaditya(`https://alfians-api.herokuapp.com/api/chord?q=${tels}`, {method: 'get'})
				...
				case 'infogempa':
					...
					if (isLimit(sender)) return reply(limitend(pushname2))
					try {
					anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/infogempa?apikey=${TobzApi}`, {method: 'get'})
				...
				case 'kucing':
					...
					if (isLimit(sender)) return reply(limitend(pushname2))
					try {
					anu = await fetchaditya(`https://api.vhtear.com/randomcat?apikey=${VthearApi}`, {method: 'get'})
					reply(mess.wait)
			...
			case 'anime':
				...
				try {
					anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/randomanime?apikey=${TobzApi}`, {method: 'get'})
					reply(mess.wait)
				...
				case 'megumin':
					...
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					try {
					anu = await fetchaditya(`http://lolhuman.herokuapp.com/api/random/megumin?apikey=${LolApi}`, {method: 'get'})
					reply(mess.wait)
					pok = await getBuffer(anu.result)
			...
			case 'animekiss':
				...
				if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/kiss?apikey=${TobzApi}`, {method: 'get'})
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						buffer = fs.readFileSync(rano)
						frhan.sendMessage(from, buffer, sticker, {quoted: mek})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					try {
					reply(mess.wait)
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=Naruto`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=Minato`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=Boruto`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=Hinata`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=sasuke`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=sakura`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.vhtear.com/pinterest?query=kaneki&apikey=${VthearApi}`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anime+touka`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anime+rize`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anime+akira`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anime+itori`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anime+karumi`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					try {
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anime+miku`, {method: 'get'})
...
// akhir fitur anime
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchaditya(`https://api.fdci.se/rep.php?gambar=anjing`, {method: 'get'})
					reply(mess.wait)
					try {
				...
				case 'resepmasakan':
					...
					if (isLimit(sender)) return reply(limitend(pushname2))
					try {
					anu = await fetchaditya(`https://api.vhtear.com/resepmasakan?query=${body.slice(14)}&apikey=${VthearApi}`, {method: 'get'})
				...
				case 'cersex':
					...
					if (isLimit(sender)) return reply(limitend(pushname2))
					try {
					anu = await fetchaditya(`https://api.vhtear.com/cerita_sex&apikey=${VthearApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'randomkpop':
					...
					if (isLimit(sender)) return reply(limitend(pushname2))
					try {
					anu = await fetchaditya(`http://lolhuman.herokuapp.com/api/random/exo?apikey=${LolApi}`, {method: 'get'})
				...
				case 'playstore':
					...
					try {
					ps = `${body.slice(11)}`
					anu = await fetchaditya(`https://api.vhtear.com/playstore?query=${ps}&apikey=${VthearApi}`, {method: 'get'})
					store = '======================\n'
				...
				case 'pornhub':
					...
					teks = body.slice(9)
					anu = await fetchaditya(`https://api.arugaz.my.id/api/media/pornhub/search?query=${teks}`, {method: 'get'})
					teks = `===============\n`
				...
				case 'nekopoi':
					...
					if (args.length < 1) return reply('teksnya mana gan?')
					teks = body.slice(9)
					anu = await fetchaditya(`https://api.vhtear.com/nekosearch?query=${teks}&apikey=${VthearApi}`, {method: 'get'})
					teks = `===============\n`
				...
				case 'xvideos':
					...
					reply(mess.wait)
					if (args.length < 1) return reply('teksnya mana gan?')
					anu = await fetchaditya(`https://api.arugaz.my.id/api/media/xvideo/search?query=${body.slice(9)}`, {method: 'get'})
					teks = `===============\n`
				...
				case 'xnxx':
					...
					anu = await fetchaditya(`https://api.arugaz.my.id/api/media/xnxx/search?query=${body.slice(6)}`, {method: 'get'})
					teks = `===============\n`
				...
				reply(mess.wait)
					...
					reply(mess.wait)
					anu = await fetchaditya(`https://mhankbarbar.tech/api/epbe?url=${args[0]}&apiKey=${BarBarApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				anu = await fetchaditya(`https://api.vhtear.com/igstory?query=${instor}&apikey=${VthearApi}`, {method: 'get'})
				insta = '=========================\n'
			...
			case 'hekerbucin':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					try {
					anu = await fetchaditya(`https://api.zeks.xyz/api/yts?q=${body.slice(10)}&apikey=${ZeksApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = '=======================\n'
				...
				try {
				reply(mess.wait)
				anu = await fetchaditya(`http://www.omdbapi.com/?s=${body.slice(6)}&plot=full&apikey=56b1b6f0&r=json`, {method: 'get'})
				hasil = '=========================\n'
...
//creator
			...
			const	textnya = teks.split("/")[2];
					...
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, mess.success)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (args.length < 1) {
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(anu), image, adityaGans, adityaGans2)
					} else {
			...
			case 'ttp':
				...
				try {
					...
					anu = await fetchaditya(`https://mhankbarbar.tech/api/text2image?text=${teks}&apiKey=${BarBarApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				reply(mess.wait)
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, `~ Emoji Maker`)
					} else {
					reply(mess.error.bug)
					}
					 await limitAdd(sender)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, ` ~ Game Logo Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, ` ~ Facebook Gold Button Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, ` ~ Black Pink Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, ` ~ Party Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, ` ~ Style Logo Text Maker`)
					} else {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(buff), image, adityaGans, ` ~ Wet Glass Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'croman':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, adityaGans, ` ~ Romance Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'clove':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, adityaGans, ` ~ Love Message Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender)
				...
				reply(mess.wait)
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, adityaGans, ` ~ Harta Tahta ${args[0]}`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				try {
					...
					reply(mess.wait)
					anu = await fetchaditya(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					costum(buffer, image, adityaGans, ` ~ Quotes Maker`)
					} catch {
					reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'cphlogo':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, adityaGans, ` ~ Pornhub Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				case 'cwolf':
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, adityaGans, ` ~ Wolf Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(anu), image, adityaGans, ` ~ Glitch Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					vuss = await ftype.fromStream(voss.body)
					if (vuss !== undefined) {
					costum(await getBuffer(anu), image, adityaGans, ` ~ Ballom Text Maker`)
					} else {
						reply(mess.error.bug)
					}
					await limitAdd(sender) 
...
//akhir kreator
			    ...
			    ja = jarak.split("/")[0];
			    rak = jarak.split("/")[1];
			    anu = await fetchaditya(`https://api.vhtear.com/distance?from=${ja}&to=${rak}&apikey=${VthearApi}`, {method: 'get'})
			    frhan.sendMessage(from, `${anu.result.data}`, text, {quoted: mek})
			    } catch {
					reply(mess.error.bug)
					}
			    ...
			    if (isLimit(sender)) return reply(limitend(pushname2))
			    reply(mess.wait)
                    anu = await fetchaditya(`https://api.vhtear.com/infoalamat?query=${body.slice(12)}&apikey=${VthearApi}`, {method: 'get'})
			        frhan.sendMessage(from, `${anu.result.data}`, text, {quoted: mek})
			        } catch {
					reply(mess.error.bug)
					}
			    ...
			    if (isLimit(sender)) return reply(limitend(pushname2))
			    reply(mess.wait)
                    anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/tinyurl?url=${body.slice(9)}&apikey=${TobzApi}`)
			        tinyurl = `${anu.result}`
			        reply(tinyurl)
			        } catch {
					reply(mess.error.bug)
			   ...
			   try {
                    if (isBanned) return reply(mess.only.benned)    
   					if (!isUser) return reply(mess.only.userB)
   					if (isLimit(sender)) return reply(limitend(pushname2))
                    anu = await fetchaditya(`https://api.vhtear.com/igprofile?query=${args[0]}&apikey=${VthearApi}`, {method: 'get'})
			    ...
			    reply(mess.wait)
			        anu = await fetchaditya(`https://api.zeks.xyz/api/artimimpi?apikey=${ZeksApi}&q=${body.slice(7)}`, {method: 'get'})
				...
				frhan.updatePresence(from, Presence.composing) 
				 ...
				 if (isLimit(sender)) return reply(limitend(pushname2))
				if (!isUser) return reply(mess.only.userB)
				 data = fs.readFileSync('./aditya/quotes.js');
				...
				if (!isUser) return reply(mess.only.userB)
					...
					anu = await fetchaditya(`http://indonesian-java-security.ezyro.com/Fakta.php?apikey=${adityaApi}`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchaditya(`https://api.arugaz.my.id/api/random/text/katabijak`, {method: 'get'})
			...
			try {
			    ...
			    if (isLimit(sender)) return reply(limitend(pushname2))
			    reply(mess.wait)
                    anu = await fetchaditya(`https://api.vhtear.com/tiktokprofile?query=${body.slice(14)}&apikey=${VthearApi}`, {method: 'get'})
		    ...
		    case 'darkjokes':
				...
				reply(mess.wait)
				 data = fs.readFileSync('./aditya/drak.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				sa = pa.split("/")[0];
				ngan = pa.split("/")[1];
				anu = await fetchaditya(`https://api.zeks.xyz/api/primbonjodoh?apikey=${ZeksApi}&nama1=${sa}&nama2=${ngan}`, {method: 'get'})
		    ...
		    try {
			    ...
			    if (isLimit(sender)) return reply(limitend(pushname2))
			    reply(mess.wait)
                anu = await fetchaditya(`https://mnazria.herokuapp.com/api/maps?search=${body.slice(5)}`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					reply(mess.wait)
					anu = await fetchaditya(`https://mhankbarbar.tech/api/url2image?tipe=${args[0]}&url=${args[1]}&apiKey=${BarBarApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 aditya BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 aditya BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    try {
			    if (isBanned) return reply(mess.only.benned)    
			    ...
			    reply(mess.wait)
					...
					anu = await fetchaditya(`https://mnazria.herokuapp.com/api/kbbi?search=${body.slice(6)}`, {method: 'get'})
					reply('Menurut Kbbi:\n\n'+anu.result)
					} catch {
					reply(mess.error.bug)
					}
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (args.length < 1) return reply('Apa yang mau dicari gan?')
					anu = await fetchaditya(`https://api.zeks.xyz/api/artinama?apikey=${ZeksApi}&nama=${body.slice(10)}`, {method: 'get'})
					frhan.sendMessage(from, anu.result, text, {quoted: mek})
					} catch {
					reply(mess.error.bug)
					}
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						costum(buffer, image, adityaGans, adityaGans2)
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
				...
				data = fs.readFileSync('./aditya/caklontong.js');
				cak = JSON.parse(data);
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					data = fs.readFileSync('./aditya/tebakgambar.js');
					jsonData = JSON.parse(data);
					randIndex = Math.floor(Math.random() * jsonData.length);
					randKey = jsonData[randIndex];
				...
				data = fs.readFileSync('./aditya/family100.js');
				...
				if (!isUser) return reply(mess.only.userB)
				 ...
				 data = fs.readFileSync('./aditya/18.js');
				 jsonData = JSON.parse(data);
				 randIndex = Math.floor(Math.random() * jsonData.length);
				 randKey = jsonData[randIndex];
...
//setting bot
				...
				case 'setreply':
					...
					if (args.length < 1) return
					adityaGans2 = args[0]
					setting.adityaGans2 = adityaGans2
					fs.writeFileSync('./database/json/settings.json', JSON.stringify(setting, null, '\t'))
...
////////////
			...
			try {
			 ...
			 frhan.updatePresence(from, Presence.composing) 
			 if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchaditya(`https://api.banghasan.com/quran/format/json/acak`, {method: 'get'})
				...
				try {
					...
					weather = await fetchaditya('https://videfikri.com/api/cuaca/?daerah='+ tempat, {method: 'get'})
					if (weather.error) {
         ...
         if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					pinte = body.slice(11)
					anu = await fetchaditya(`https://api.vhtear.com/pinterest?query=${pinte}&apikey=${VthearApi}`, {method: 'get'})
					reply(mess.wait)
			...
			case 'instaimg':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					anu = await fetchaditya(`http://lolhuman.herokuapp.com/api/sholat/${sholat}?apikey=${LolApi}`, {method: 'get'})
					reply(mess.wait)
				...
				case 'jadwaltv':
					...
					anu = await fetchaditya(`http://nzcha-apii.herokuapp.com/jadwaltv?channel=${jadwaltv}`, {method: 'get'})
					jtv = '===========================\n'
            ...
            case 'jadwaltvnow':
				...
				frhan.updatePresence(from, Presence.composing) 
				if (isLimit(sender)) return reply(limitend(pushname2))
				anu = await fetchaditya(`http://lolhuman.herokuapp.com/api/jadwaltv/now?apikey=${LolApi}`, {method: 'get'})
				reply(mess.wait)
...
// premium user
         ...
         try {
			if (isBanned) return reply(mess.only.benned)    
                ...
                anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/joox?q=${body.slice(6)}&apikey=${TobzApi}`, {method: 'get'})
               if (anu.error) return reply(anu.error)
          ...
          case 'snack':
			...
			if (isBanned) return reply(mess.only.benned)    
                ...
                anu = await fetchaditya(`https://api-anoncybfakeplayer.herokuapp.com/sckdown?url=${args[0]}`, {method: 'get'})
               if (anu.error) return reply(anu.error)
                 sck = `「 *SNACK VIDEO DOWNLOADER* 」\n\n*• Format:* ${anu.format}\n*• Size:* ${anu.size}\n\n*TUNGGU SEBENTAR LAGI DIKIRIM MOHON JANGAN SPAM*`
                bufferddd = await getBuffer('https://raw.githubusercontent.com/adityaXCode7/termux-bot-wa/main/src/glitchtext.png')
                 reply(mess.wait)
                buff = await getBuffer(anu.result)
       ...
       case 'ytmp4':
					...
					anu = await fetchaditya(`https://mhankbarbar.tech/api/ytv?url=${args[0]}&apiKey=${BarBarApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				anu = await fetchaditya(`https://api.vhtear.com/ytmp3?query=${playmp3}&apikey=${VthearApi}`, {method: 'get'})
			...
			case 'ytmp3':
				...
				reply(mess.wait)
				playmp3 = body.slice(9)
				anu = await fetchaditya(`https://naufalhoster.xyz/dl/youtube?apikey=Cv5SHS-9ZxAto-HnWqLR&url=${args[0]}`, {method: 'get'})
				mp3 = `${anu.result}`
			...
			case 'smule':
				...
				reply(mess.wait)
				anu = await fetchaditya(`https://mnazria.herokuapp.com/api/smule?link=${args[0]}`, {method: 'get'})
				if (anu.error) return reply(anu.error)
...
// akhir Fitur Premium
			...
			frhan.updatePresence(from, Presence.composing) 
				...
				reply(mess.wait)
				data = fs.readFileSync('./aditya/asupan.js');
				jsonData = JSON.parse(data);
				randIndex = Math.floor(Math.random() * jsonData.length);
				randKey = jsonData[randIndex];
			...
			case 'wiki':
				...
				reply(mess.wait)
				wiki = `${body.slice(6)}`
				anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/wiki?q=${wiki}&apikey=${TobzApi}`, {method: 'get'})
				if (anu.error) return reply(anu.error)
				wikii = `${anu.result}`
			...
			case 'pastebin':
				...
				reply(mess.wait)
				paste = `${body.slice(10)}`
                   anu = await fetchaditya(`https://api-anoncybfakeplayer.herokuapp.com/pastebin?text=${paste}`, {method: 'get'})
                   frhan.sendMessage(from, `${anu.result}`, text, {quoted: mek})
                   } catch {
					reply(mess.error.bug)
					}
		...
		case 'bpfont':
			...
			bp = `${body.slice(8)}`
			anu = await fetchaditya(`https://api.terhambar.com/bpk?kata=${bp}`, {method: 'get'})
			reply (anu.text)
			} catch {
					reply(mess.error.bug)
					}
		...
		case 'spamcall':
			...
			call = `${body.slice(11)}`
			anu = await fetchaditya(`https://videfikri.com/api/call/?nohp=${call}`, {method: 'get'})
		...
		case 'spamgmail':
			...
			if (isLimit(sender)) return reply(limitend(pushname2))
			spam = `${body.slice(10)}`
			anu = await fetchaditya(`https://videfikri.com/api/spamemail/?email=${spam}&subjek=PT.PLN&pesan=Silahkan%20bayar%20tagihan%20listrik%20Anda`, {method: 'get'})
		...
		case 'quransurah':
			...
			reply(mess.wait)
			surah = `${body.slice(12)}`
			anu = await fetchaditya(`https://api.zeks.xyz/api/quran?no=${surah}&apikey=${ZeksApi}`, {method: 'get'})
		...
		case 'bitly':
			...
			if (isLimit(sender)) return reply(limitend(pushname2))
			link = `${body.slice(7)}`
			anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/bitly?url=${link}&apikey=${TobzApi}`, {method: 'get'})
			bitly = `${anu.result}`
			...
			reply(mess.wait)
			style = `${body.slice(11)}`
			anu = await fetchaditya(`https://api.arugaz.my.id/api/random/text/fancytext?text=${style}`, {method: 'get'})
			reply (anu.result)
			} catch {
					reply(mess.error.bug)
					}
			...
			if (isBanned) return reply(mess.only.benned)
			if (!isUser) return reply(mess.only.userB)
			anu = await fetchaditya(`https://api.zeks.xyz/api/pantun?apikey=${ZeksApi}`, {method: 'get'})
		...
		if (isLimit(sender)) return reply(limitend(pushname2))
		 ...
		 jamdunia = `${body.slice(10)}`
			anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/jamdunia?lokasi=${jamdunia}&apikey=${TobzApi}`, {method: 'get'})
...
case 'whois':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				whois = body.slice(7)
				anu = await fetchaditya(`https://api.banghasan.com/domain/whois/${whois}`, {method: 'get'})
				reply(anu.hasil)
				} catch {
					reply(mess.error.bug)
					}
...
case 'hostsearch':
				...
				anu = await fetchaditya(`https://api.banghasan.com/domain/hostsearch/${hostsearch}`, {method: 'get'})
				reply(anu.hasil)
				} catch {
					reply(mess.error.bug)
					}
...
case 'dnslookup':
				...
				anu = await fetchaditya(`https://api.banghasan.com/domain/dnslookup/${dnslookup}`, {method: 'get'})
				reply(anu.hasil)
				} catch {
					reply(mess.error.bug)
					}
...
case 'geoip':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				geoip = body.slice(7)
				anu = await fetchaditya(`https://api.banghasan.com/domain/geoip/${geoip}`, {method: 'get'})
				reply(anu.hasil)
				} catch {
					reply(mess.error.bug)
					}
...
case 'nping':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				nping = body.slice(7)
				anu = await fetchaditya(`https://api.banghasan.com/domain/nping/${nping}`, {method: 'get'})
				reply(anu.hasil)
				} catch {
					reply(mess.error.bug)
					}
			...
			break 
				...
				reply(mess.wait)
				dork = `${body.slice(9)}`
					anu = await fetchaditya(`https://api-anoncybfakeplayer.herokuapp.com/dorking?dork=${dork}`, {method: 'get'})
					hasil = `${anu.result}`
					frhan.sendMessage(from, hasil, text, {quoted: mek})
					} catch {
					reply(mess.error.bug)
			...
			case 'encode64':
				...
				anu = await fetchaditya(`https://api.vhtear.com/encode_string?string=${encode64}&apikey=${VthearApi}`, {method: 'get'})
			...
			case 'decode64':
				...
				anu = await fetchaditya(`https://api.vhtear.com/decode_string?string=${decode64}&apikey=${VthearApi}`, {method: 'get'})
			...
			case 'hexaencode':
				...
				anu = await fetchaditya(`https://api.vhtear.com/hex_to_text?string=${hexaencode}&apikey=${VthearApi}`, {method: 'get'})
			...
			case 'hexadecode':
				...
				anu = await fetchaditya(`https://api.vhtear.com/hex_to_text?string=${hexadecode}&apikey=${VthearApi}`, {method: 'get'})
			...
			break 
				...
				encbinary = `${body.slice(11)}`
					anu = await fetchaditya(`https://api.anoncybfakeplayer.com/api/binary/?encode=${encbinary}`, {method: 'get'})
					frhan.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					} catch {
					reply(mess.error.bug)
					}
				...
				decbin = `${body.slice(11)}`
					anu = await fetchaditya(`https://api.anoncybfakeplayer.com/api/binary/?decode=${decbin}`, {method: 'get'})
					frhan.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					} catch {
					reply(mess.error.bug)
					}
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				encoc = `${body.slice(10)}`
					anu = await fetchaditya(`https://api.anoncybfakeplayer.com/api/octal/?encode=${encoc}`, {method: 'get'})
					frhan.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					} catch {
					reply(mess.error.bug)
					}
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				decoc = `${body.slice(10)}`
					anu = await fetchaditya(`https://api.anoncybfakeplayer.com/api/octal/?decode=${decoc}`, {method: 'get'})
					frhan.sendMessage(from, `${anu.result}`, text, {quoted: mek})
					} catch {
					reply(mess.error.bug)
					}
			...
			case 'hashidentifier':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					  hash = `${body.slice(16)}`
					  anu = await fetchaditya(`https://freerestapi.herokuapp.com/api/v1/hash-identifier?hash=${hash}`)
...
// akhir encrypt & decrypt Fitur
				...
				case 'moddroid':
					...
					if (!isUser) return reply(mess.only.userB)
					if (isLimit(sender)) return reply(limitend(pushname2))
					anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/moddroid?q=${body.slice(10)}&apikey=${TobzApi}`, {method: 'get'})
		...
		case 'happymod':
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limitend(pushname2))
			anu = await fetchaditya(`https://tobz-api.herokuapp.com/api/happymod?q=${body.slice(10)}&apikey=${TobzApi}`, {method: 'get'})
		...
		case 'tiktoksearch':
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
			anu = await fetchaditya(`https://api.vhtear.com/tiktokhastag?query=${body.slice(14)}&apikey=${VthearApi}`, {method: 'get'})
			tts = '====================================\n'
		...
		case 'wattpad':
			...
			reply(mess.wait)
			wttpd = `${body.slice(9)}`
			anu = await fetchaditya(`http://nzcha-apii.herokuapp.com/wattpad-search?q=${wttpd}`, {method: 'get'})
			wattp = '===========================\n'
		...
		case 'tanggaljadian':
			...
			di = tggl.split("/")[1];
			an = tggl.split("/")[2];
			anu = await fetchaditya(`https://videfikri.com/api/primbon/tgljadian/?tgl=${ja}&bln=${di}&thn=${an}`, {method: 'get'})
		...
		break 
			case 'zodiak':
				...
				anu = await fetchaditya(`https://api.vhtear.com/zodiak?query=${body.slice(8)}&apikey=${VthearApi}`, {method: 'get'})
			...
			case 'blackpink':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				anu = await fetchaditya(`https://api.shizukaa.xyz/blackpink?apikey=itsmeiky633`, {method: 'get'})
				buff = await getBuffer(anu.result)
			...
			case 'randombts':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				anu = await fetchaditya(`http://lolhuman.herokuapp.com/api/random/bts?apikey=${LolApi}`, {method: 'get'})
				buff = await getBuffer(anu.result)
				frhan.sendMessage(from, buff, image, {quoted: mek, caption: 'BTS.....'})
					...
					await limitAdd(sender)
			case 'antivirtexx':
			await costum(antivirtexx(), text, adityaGans, `Buset Dahh Etekel🔥`)
			break 
			case 'addstatus':
					if (!isOwner) return reply(mess.only.ownerB)
